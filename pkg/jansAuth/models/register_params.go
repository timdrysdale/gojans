...
func (m *RegisterParams) validateJWKS(formats strfmt.Registry) error {

	if swag.IsZero(m.JWKS) { // not required
		return nil
	}

	for i := 0; i < len(m.JWKS); i++ {
		if swag.IsZero(m.JWKS[i]) { // not required
			continue
		}

		if m.JWKS[i] != nil {
			if err := m.JWKS[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("jwks" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *RegisterParams) validateRedirectUris(formats strfmt.Registry) error {
